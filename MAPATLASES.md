...
own request to open one (its `C2S2COpenAtlasScreenPacket`, on Map Atlases' own network channel) — a
...
- **Save pin in Map Atlases** — deliberately copies the destination into Map Atlases' own personal-pin
  ...
  and follows Map Atlases' own persistence. Controlled by `mapAtlasesPins`. The button always appears
  for Map Atlases, even when its native pin feature (which depends on Moonlight) is unavailable; it is
  then shown disabled with a reason instead of being left out of the list.
...
(Twilight Forest magic maps, in the inspected release) shows "Quest markers are hidden on this map
type" instead of an unverified drawing.
...
promise this document can make ahead of it.
