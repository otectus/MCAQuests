...
### `[client]`
These keys live in `config/mcaquests-client.toml`, registered as a NeoForge `CLIENT` config — per-instance like the common file, not per-world. The quest tracker keys below are read fresh every frame the HUD is drawn, so an edit to any of them applies as soon as the file is saved, with no restart and no world reload.
