...
The entire pack mounts only when `iceandfire.core` is present.
...
Suspension is decided fresh every pass rather than written into the save, so recovery needs no migration and nothing can go stale. Offers simply stop appearing because the bundled pack mounts only when `iceandfire.core` is present.
