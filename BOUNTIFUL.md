...
Bountiful has a public API for everything except bounty completion. MCA: Quests reads bounty properties and mounts conditional pools and decrees through the normal datapack loader. Completion is the one thing the API does not expose: when a player cashes in a bounty at the board, nothing tells MCA: Quests about it — so the integration includes an optional Mixin observer (`BountyDataCashInMixin`, a single method hook on Bountiful's `BountyData.tryCashIn`) that is applied only when Bountiful is installed and the method has the expected shape.
...
- **contractor** (any profession) — complete two bounties from the board; gated on `bountiful.cash_in_hook`
- **specialist** (any profession) — complete one high-rarity bounty; gated on both `bountiful.cash_in_hook` and `bountiful.read_rarity`
...
**Why can't I find the decree in a board?** The decree is not wired into any board or loot table; it has to be placed manually. A server admin can add it to a board through Bountiful's own UI or commands (`/bountiful decree add ...`).

**Does the client need Bountiful?** The mixin is applied wherever Bountiful's `BountyData` class loads (both sides when Bountiful is installed), and the handler only acts for server-side players. A client without Bountiful sees no behaviour change; a client with Bountiful sees no change either because bounty-completion is a server-side observable.
