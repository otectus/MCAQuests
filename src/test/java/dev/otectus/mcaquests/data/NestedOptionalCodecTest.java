...
class NestedOptionalCodecTest {
    ...
    void nestedPaymentAndEligibilityFieldsRejectInvalidPresentValues() {
        for (Malformed fixture : List.of(
                ...
                new Malformed(ItemDeliveryObjective.CODEC, "{\"item\":\"minecraft:apple\",\"count\":0}"),
                new Malformed(ItemDeliveryObjective.CODEC, "{\"item\":\"minecraft:apple\",\"consume\":\"typo\"}"),
                new Malformed(DeliverToVillagerObjective.CODEC, "{\"recipient\":{\"mode\":\"self\"},\"item\":\"minecraft:apple\",\"count\":-1}"),
                new Malformed(DeliverToVillagerObjective.CODEC, "{\"recipient\":{\"mode\":\"self\"},\"item\":\"minecraft:apple\",\"destination\":42}"),
                new Malformed(ObtainItemObjective.CODEC, "{\"item\":\"minecraft:apple\",\"count\":\"typo\"}"),
                new Malformed(DonateItemObjective.CODEC, "{\"item\":\"minecraft:apple\",\"per_player_cap\":-1}"),
                new Malformed(ItemReward.CODEC, "{\"item\":\"minecraft:apple\",\"count\":0}"),
                new Malformed(CurrencyReward.CODEC, "{\"difficulty\":\"typo\"}"),
                new Malformed(EffectReward.CODEC, "{\"effect\":\"minecraft:speed\",\"duration\":0}"),
                new Malformed(HeartsCondition.CODEC, "{\"min\":\"typo\"}"),
                new Malformed(QuestCompletedCondition.CODEC, "{\"quest\":\"test:previous\",\"scope\":\"typo\"}"),
    ...
    void omittedNestedDefaultsAndValidDefinitionsStillRoundTrip() {
        ...
        ItemDeliveryObjective delivery = parse(ItemDeliveryObjective.CODEC, "{\"item\":\"minecraft:apple\"}");
        ...
        assertTrue(parse(HeartsCondition.CODEC, "{}").min().isEmpty());
        assertTrue(parse(HeartsCondition.CODEC, "{}").max().isEmpty());
    ...
    void unknownDefaultedRegistryEntriesDoNotBecomeAir() {
        assertRejected(ItemDeliveryObjective.CODEC, "{\"item\":\"missingmod:unknown\"}");
        assertRejected(ItemReward.CODEC, "{\"item\":\"missingmod:unknown\"}");
