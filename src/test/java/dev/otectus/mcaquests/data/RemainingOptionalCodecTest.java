...
class RemainingOptionalCodecTest {
    ...
    void malformedTriggerFiltersDoNotBroadenSignals() {
        assertRejected(LowFoodTrigger.CODEC.codec(), "{\"threshold\":\"typo\"}");
        assertRejected(VillagerDeathTrigger.CODEC.codec(), "{\"relation\":[]}");
        assertRejected(MissingKinTrigger.CODEC.codec(), "{\"relation\":{}}");
        assertRejected(NightTrigger.CODEC.codec(), "{\"require_full_moon\":\"typo\"}");
        assertRejected(InfectionTrigger.CODEC.codec(), "{\"min_progress\":2.0}");
        assertEquals(16, parse(LowFoodTrigger.CODEC.codec(), "{}").threshold());
        assertEquals("any", parse(VillagerDeathTrigger.CODEC.codec(), "{}").relation());
        assertEquals("any", parse(MissingKinTrigger.CODEC.codec(), "{}").relation());
        assertFalse(parse(NightTrigger.CODEC.codec(), "{}").requireFullMoon());
        assertEquals(0.0f, parse(InfectionTrigger.CODEC.codec(), "{}").minProgress());
    }
