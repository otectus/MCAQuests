...
class MapAtlasesJarProbeTest {
    ...
    @Test void releaseHasCompleteRenderAndNativeActionContracts() throws Exception {
        ...
        try(var jar=new JarFile(path)) {
            ...
            assertNotNull(AtlasHookManifest.method(pins,"addPin","(Lpepjebs/mapatlases/utils/MapDataHolder;Lnet/minecraft/server/level/ColumnPos;Ljava/lang/String;I)V"));
            assertTrue(pins.fields.stream().anyMatch(f->f.name.equals("markersPerMap")));
        ...
        try(var jar=new JarFile(moon)) {
            var channel=node(jar,"net.mehvahdjukaar.moonlight.api.platform.network.ChannelHandler");
            assertNotNull(AtlasHookManifest.method(channel,"sendToServer","(Lnet/mehvahdjukaar/moonlight/api/platform/network/Message;)V"));
        }
