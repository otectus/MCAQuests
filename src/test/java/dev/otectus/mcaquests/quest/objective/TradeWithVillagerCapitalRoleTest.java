...
class TradeWithVillagerCapitalRoleTest {
    ...
    private static TradeWithVillagerObjective parse(String json) {
        return TradeWithVillagerObjective.CODEC.parse(JsonOps.INSTANCE, JsonParser.parseString(json))
                .result().orElseThrow();
    }
    ...
    void misspelledCourtRoleCannotSilentlyBecomeATradeWithAnybody() {
        assertTrue(TradeWithVillagerObjective.CODEC.parse(JsonOps.INSTANCE, JsonParser.parseString(
