...
public final class AtlasHeldOverlay {
    private static final RenderType INK = RenderType.text(new ResourceLocation("mcaquests", "textures/gui/atlas_ink.png"));
    ...
    private static void quad(PoseStack pose, MultiBufferSource buffers, int light,
        ...
        ink.vertex(matrix,left,bottom,z).color(color).uv(0,1).uv2(light).endVertex();
        ink.vertex(matrix,right,bottom,z).color(color).uv(1,1).uv2(light).endVertex();
        ink.vertex(matrix,right,top,z).color(color).uv(1,0).uv2(light).endVertex();
        ink.vertex(matrix,left,top,z).color(color).uv(0,0).uv2(light).endVertex();
    }
