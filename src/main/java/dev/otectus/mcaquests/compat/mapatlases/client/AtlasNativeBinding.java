...
public final class AtlasNativeBinding {
    ...
    private final Class<?> collectionType = type("map_collection.IMapCollection");
    ...
    private final Method all = method(collectionType, "getAll"),
            ...
            hasMarkers = method(type("utils.MapType"), "hasMarkers"),
    ...
    public AtlasNativeBinding() {
        try {
            ...
            channel = field(type("networking.MapAtlasesNetworking"), "CHANNEL").get(null);
            send = channel.getClass().getMethod("sendToServer", typeAbsolute("net.mehvahdjukaar.moonlight.api.platform.network.Message"));
        } catch (ReflectiveOperationException | LinkageError | RuntimeException error) {
            navigationFailure = error.getClass().getSimpleName();
        }
        try {
            Class<?> pins = type("integration.moonlight.ClientMarkers");
            addPin = method(pins, "addPin", holderType, ColumnPos.class, String.class, int.class);
            pinType = method(pins, "getPinWithIndex", int.class);
            nativePins = field(pins, "markersPerMap");
    ...
    public Tile tile(Object holder) {
        ...
        return new Tile(holder, (MapItemSavedData) read(data, holder), (Integer) read(height, holder),
                ((Enum<?>) type).name(), read(slice, holder), (boolean) invoke(hasMarkers, type));
    }
