...
| `mcaquests:heal_entity` | `villager` (default self), item target (req.), `below_health_fraction` (0–1, def 1.0), `count` (def 1), `consume` (bool, def false) | Use a remedy item on a hurt villager. A villager at **full health never credits**, whatever `below_health_fraction` is set to — the default of `1.0` means "hurt at all," not "any health." An unreadable health fraction is treated as full and does not credit either. |
...
| `mcaquests:sleep_or_rest` | `require_morning` (bool, def true) | Sleep, or rest. With `require_morning:true` (default) the credit comes when the night is actually skipped for everyone; with `require_morning:false` it comes when the player **gets out of bed** after sleeping long enough, whether or not morning has come — but a bounce-out (a disconnect, a dimension change, a bed made unusable underneath the player) never credits either way. Guidance points at the player's own respawn bed, and only calls it a bed when a loaded, unforced bed block is actually there; otherwise it gives a plain "sleep in a bed" instruction instead of pointing at a mined-out bed, a respawn anchor, or a forced `/spawnpoint` location. |
...
| Key | Default | Notes |
|---|---|---|
| `enableVillageProjects` | `true` | Master switch for creating and advancing projects — offers, contributions and phase advancement. It does not stop delivery of rewards already owed: a banked FTB-claim reward (reputation, hearts, titles) still pays out on login while this is off. A pending project-phase reward is held rather than paid until it is turned back on. |
...
| `/mcaquests project pending <player>` | 2 | List a player's undelivered rewards, including any held after repeated delivery failures. |
| `/mcaquests project pending <player> retry` | 3 | Clear the delivery attempt counts and retry — immediately if the player is online, on next login otherwise. |
...
version range `[1.3.5,)` and itself requires MCA Reborn 7.7.1-alpha.2+ — see [CAPITALS.md](CAPITALS.md) for the
