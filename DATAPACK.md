...
| `type` | Fields | Meaning |
|---|---|---|
| `mcaquests:item_delivery` | item target, `count`, `consume` (bool, default `true`), `destination` (optional) | Bring items; consumed on turn-in unless `consume:false`. Since 1.6.5 the player may also hand them over **early** — the **Deliver** button on the quest card, or MCA's **Gift** gesture — and those units are recorded and never charged again, so completion is *deposits plus what is still carried*. Partial hand-ins are fine, and surplus is never taken. From 1.6.5 only the hotbar and main inventory count towards progress and may be taken: an item that is worn or in the offhand reads as short until it is moved into the pack, rather than being taken silently. (A pure **proof** objective takes nothing, so it still counts the whole inventory.) A `consume:false` objective with no `destination` is a pure **proof** objective: it is answered with **Show items**, nothing is taken, and it still asks whether all of the goods are carried right now. **`destination`** sends the goods somewhere instead of destroying them: `{"type": "townstead_villager_inventory", "target": "giver"}` puts them in the villager's own inventory, where [Townstead](TOWNSTEAD.md) lets them be eaten or used. A destination overrides `consume`, and a hand-over is **refused** rather than half-done when the goods will not fit. The default, `consume`, is the behaviour every existing pack already has. |
...
| `mcaquests:heal_entity` | `villager` (default self), item target (req.), `below_health_fraction` (0–1, def 1.0), `count` (def 1), `consume` (bool, def false) | Use a remedy item on a hurt villager. A villager at **full health never qualifies**, whatever `below_health_fraction` is set to — the default of `1.0` means "hurt at all", not "any health at or below full". A villager whose health cannot be read also does not qualify. |
...
| `mcaquests:sleep_or_rest` | `require_morning` (bool, def true) | Sleep, or rest. `require_morning:true` (default) credits when the night is skipped for everyone; `require_morning:false` credits when the player gets up after sleeping long enough, without waiting for morning. A bounce-out — disconnect, dimension change, a bed made unusable underneath the player — never credits either way. |
...
| `dimension` | Point at **the way in** — the nearest lit nether portal, or the nearest stronghold for the End — and stop pointing once the player is through. Not the dimension itself, which is not a place you can walk to. When no portal can be found in the player's current world, this shows a text-only instruction naming the destination's dimension instead of falling back to search the anchor/block/structure/biome fields in the wrong world. |
...
`entries` must be a non-empty list — an empty list fails the quest's load. Each entry is `item` (required), `count` (default `1`), `weight` (default `1`), and an optional `enchantments` map with the same level semantics as `mcaquests:item`'s.

**Enchantment ids resolve lazily, not at load.** Because enchantments are a datapack registry in this port, an id authored in `enchantments` (on either `mcaquests:item` or an `item_pool` entry) cannot be checked while the quest JSON is parsed — the registry does not exist yet. It parses either way; an unknown enchantment id is simply skipped when the reward is actually built (granted or previewed), logging one warning per id, and `/mcaquests validate` reports it as a warning too, without failing the quest's load. An unknown `item` id is different for `mcaquests:item`, and unchanged from 1.20.1 there: it still fails the quest's load immediately. An `item_pool` entry's `item` field does not — see below.
...
Either way, from 1.6.5 the hand-over has **two routes by default** (a third, the old right-click, is
behind `legacyInteractDelivery`) and all of them pay the same ledger: the **Deliver** button on the quest
card in the recipient's Quests menu, and MCA's own **Gift** gesture, which pays one item per gift to the
villager the quest named. Deposits are per-objective, partial hand-ins are counted, and they are **not
refunded when a quest is abandoned**. Every route, final turn-in included, takes goods only from the
hotbar and the main inventory — never worn armour, never the offhand — and prefers a plain stack over a
named or enchanted one (read from the stack's data components: no custom name, no enchantments).

With it they land in the villager's real inventory, where **Townstead lets them actually be
...
| Key | Default | Notes |
|---|---|---|
| `enableVillageProjects` | `true` | Master switch for creating and advancing projects. It does not stop delivery of rewards a player is already owed — a banked FTB-claim reward pays out even while this is off; only phase rewards are held until it is back on. |
...
Without MCA: Reputation, or without a build of MCA: Reputation that includes the per-villager opinion API, the condition is never met — answering "yes" to a question nobody can answer would have villagers reacting to things they never witnessed. The per-villager opinion API is not yet available in the 1.21.1 build of MCA: Reputation, so this condition currently always evaluates false on this platform.
