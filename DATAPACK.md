...
| `mcaquests:use_item` | `item` (resource location), `count`, `require_success` (bool, default `false`), `source` (optional) | Use an item (right-click). `require_success:false` credits any use attempt; `require_success:true` credits only a finished use for items with a use duration (e.g., eating, drawing a bow). For a bow, "finished" means releasing the shot with ammunition available and the draw held long enough to loose an arrow — a tap of the button credits nothing. A crossbow already fires its finish event on **loading**, so `require_success` credits it the moment loading completes, not on the later trigger-pull. An item id that is not registered makes the quest unofferable/paused rather than failing to load. |
