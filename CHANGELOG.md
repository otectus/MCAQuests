...
- **`apiJar` Gradle task** produces `build/libs/mcaquests-<version>-api.jar`, a compile-only
  ...
  and the player quest state types) intended for sibling add-ons such as MCA: Conversations. It is
  for sibling add-ons to compile against and must never be shipped inside another mod — MCA:
  Quests is the only thing that supplies these classes at runtime, and a bundled copy is a
...
- Reward preview icons now clamp their displayed count to `64`, because the network packet that carries
  a `QuestCard` writes an item count as a single byte and a preview above `127` would wrap on the wire.
  The reward's text line still shows the true amount, and `grant` still pays the full count, split into
  proper stacks.
...
- No network protocol bump: `QuestNetwork.PROTOCOL_VERSION` stays `"15"`.
- No new mixin, and no new persisted save data — `VillageProximity`'s cache is in-memory only and
  clears on world unload, server stop, and `/reload`.
- The API jar's export list is unchanged by the item reward and tracker work in this release.
- `PendingItemRewards`' saved NBT gains a reserved `stacks` key (for enchanted and loot-table
  rewards that overflowed a full inventory); older builds ignore an unrecognized key, and a save
...
- Added a dedicated, optional, client-only mixin configuration, `mcaquests.mapatlases.mixins.json`
  ...
  (`pepjebs.mapatlases.client.AbstractAtlasWidget`) and `AtlasInHandMixin` on its held-item renderer
  (`pepjebs.mapatlases.client.AtlasInHandRenderer`). The plugin verifies an exact, version-pinned
  Forge `1.20-6.0.20` bytecode shape before applying either one, and both only inject additional
...
- Added an optional, client-side `map_atlases` dependency entry to `mods.toml`
  (`mandatory=false`, `versionRange="*"`, `side="CLIENT"`). Map Atlases is never required and never
  bundled; like the existing Xaero binding, MCA: Quests resolves its classes reflectively at runtime
  and adds no compile-time dependency on it or on its optional Moonlight dependency.
- Added Gradle support in `build.gradle`: `mapAtlasesProbeTest` replays the exact render and
  ...
  `enableCapitalsInDev`; and `-PatlasRuntimeFixture=true` applies `tools/atlas-runtime-test/forge.gradle`,
  a disposable test-mod source set for manual runtime verification, separate from every published
  artifact.
...
- **Network protocol:** No change; `QuestNetwork.PROTOCOL_VERSION` stays `"14"`. Clients and servers must match, but need not match an existing 1.5.4 installation (version skew from 1.5.4 is not a blocker). Capital state and title grants are rendered in components sent by the server.
- **MCA Capitals requirement:** MCA: Quests declares `mcacapitals` as an optional dependency with version range `[1.3.5,)`. MCA Capitals' own mods.toml requires MCA Reborn 7.7.1-alpha.2+, so a world that installs Capitals must be on that MCA Reborn version.
...
- New `-PenableCapitalsInDev=true` flag: run the dev environment with Capitals installed (`./gradlew runClient -PmcaDevVersion=7.7.1-alpha.2+1.20.1 -PenableCapitalsInDev=true`).
