...
- 🗺️ **Map Atlases integration** (optional, client-side) — draws the same automatic quest destinations on Map Atlases' own atlas screen, minimap and held-map rendering, adds a "Quest destinations" list and "Focus followed quest" action to the atlas screen, and offers explicit "Show in atlas" and "Save pin" actions alongside JourneyMap's and Xaero's. Checked against one exact, version-pinned Map Atlases release and disables itself, with a diagnosable reason, on any other build — see **[MAPATLASES.md](MAPATLASES.md)**.
...
| **[Townstead](https://www.curseforge.com/minecraft/mc-mods/townstead)** | *Optional* — `[0.7.5,0.8)`; a matching NeoForge Townstead jar still needs runtime verification; adds villager needs, professions, skills and village character as quest state ([TOWNSTEAD.md](TOWNSTEAD.md)) |
...
| **[Map Atlases](https://www.curseforge.com/minecraft/mc-mods/map-atlases)** | *Optional, client-side* — quest destinations on its atlas screen, minimap and held-atlas view; checked against one exact, version-pinned release ([MAPATLASES.md](MAPATLASES.md)) |
...
The **Map Atlases** integration (optional, client-side) draws the same automatic quest destinations on Map Atlases' own atlas screen, minimap and held-map rendering, adds a "Quest destinations" list and "Focus followed quest" action to the atlas screen, and offers explicit "Show in atlas" and "Save pin" actions alongside JourneyMap's and Xaero's. It is checked against one exact, version-pinned Map Atlases release and disables itself, with a diagnosable reason, on any other build. See **[MAPATLASES.md](MAPATLASES.md)**.

`/mcaquestsclient waypoints status` and `probe` report what bound and test the backends. `mapWaypoints` turns the whole thing off; `journeyMapWaypoints`, `xaeroWaypoints` and `mapAtlasesWaypoints` toggle each backend independently.
## License & credits
