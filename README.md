...
- 🔦 **Never lose your quest target** — follow a quest from the log and the mod answers *where next*: a **marker in the world**, visible through walls, standing on the place the objective you are actually on is about, and a tracker line naming it with a live distance and bearing ("Anna's home — 84 blocks ahead-right"). It moves by itself as each step lands — a portal until you reach the Nether, a fortress once you are there, the villager to hand it back to when you are done — and fades out as you arrive. The villager a quest wants you to reach is **outlined**, for you alone, and only while reaching them is what you are doing. Family quests bind one *specific* relative when you accept, so the name on the card, the villager that glows, and the one who takes the parcel are always the same person — and a relative who has genuinely gone **missing** is found out in the world, materialised with their real name and every family link intact.
...
- 🧩 **Extensible** — a public Java API lets add-ons register their own objective/reward/condition types (including where their objective sends the player, via `QuestObjective.guidance`), and six Forge events (`QuestAccepted/Declined/Ready/Completed/Abandoned/Failed`) let other mods react.
...
The **JourneyMap** and **Xaero's Minimap** integrations are optional, client-side, and bound the same way — by name, at runtime, with a build-time check that fails if any compiled class so much as mentions one of their types. Neither mod is redistributable and neither publishes to a maven this build can reach, so compiling against them would mean the mod could only be built by somebody who had separately downloaded two files. With either installed, the destinations the tracker already names also appear as waypoints: one per active quest that has somewhere to send you, moved as the quest advances and taken away when it ends. They are never written into your own saved waypoints — they belong to the quest, so uninstalling this mod leaves nothing behind to tidy up. `/mcaquests debug waypoints` says what bound, and `mapWaypoints` turns the whole thing off.
## License & credits
