...
> **Historical design proposal, superseded by the shipped integration.** This document predates implementation. Its Minecraft 1.16.5 references, example API names, JSON schemas, event hooks, and proposed features are not the supported contract. The implementation targets **Minecraft 1.21.1 / NeoForge / Java 21**, using optional reflective bindings to MCA Capitals. See [CAPITALS.md](../CAPITALS.md), [DATAPACK.md](../DATAPACK.md#mca-capitals-integration-optional), and [CONFIG.md](../CONFIG.md#compatcapitals) for current usage.
>
> Shipped scope: nine capabilities, five conditions, `capital_role` villager targeting, three rewards, eight quests and two polled situations. Founding houses, appointing sovereigns, changing allegiance, dialogue pledges, coronation event detection and cross-capital role targeting remain proposals, not incomplete registered features. The envoy quest addresses the giver's ambassador. The coronation gift checks an occupied throne. The historical text below is retained to explain the original design, not as an implementation checklist.
