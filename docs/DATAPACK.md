...
"dialogue": {
  ...
  "failed":      { "text": "The moment has passed, I'm afraid." },
  "cooldown":    { "text": "You brought me wheat only yesterday." },
  "locked":      { "text": "There's a thing I'd ask, but not yet." }
}
```

| State | When the villager says it |
|---|---|
| `offer` | On the offer card, before you accept |
| `accept` | You accepted |
| `decline` | You turned it down (1.4.3) |
| `in_progress` | You have it active and are not finished |
| `ready` | Every objective is satisfied |
| `complete` | You handed it in |
| `failed` | A deadline passed, or a situation resolved without you |
| `cooldown` | You did this recently, so it is not on offer |
| `locked` | It exists, but its conditions are not met yet |

`cooldown` and `locked` are what a villager says when they have **nothing** to offer. Only one is
shown, and a quest coming back soonest wins over one that is merely locked — "come back tomorrow" is
more use than "there is a thing you cannot do yet".

A quest that writes neither still gets a line, from the shared [villager voice](#villager-voice)
below. You only need these two when *this* quest deserves a bespoke refusal.
...
Shared lines a villager can say when no quest of theirs has anything more specific. One file per
situation, and every line may be gated by the **same [condition](#conditions) language quests are
gated with** — so personality, mood, time of day, weather, hearts, reputation tier, relationship
state and age group are all available to dialogue without learning anything new.
  ...
  "lines": [
    {
      ...
      "translate": "mypack.voice.cooldown.brusque"
    },
    {
      "when": { "type": "mcaquests:time", "period": "night" },
      "text": "Come back in the morning. I'm for bed.",
      "weight": 2
    },
    { "text": "You did that for me recently. Ask again in a few days." }
  ]
}
```
...
|---|---|---|
| `state` | ✅ | `cooldown`, `locked`, `no_quests`, or `greeting` |
| `priority` | — | Higher pools are consulted first; default `0` |
| `lines` | ✅ | Non-empty list |
| `lines[].when` | — | Any condition, including composites. Omit for a fallback line |
| `lines[].text` / `translate` / `with` | ✅ | A [text value](#text-values), inline on the line |
| `lines[].weight` | — | Relative likelihood among matching lines; default `1` |

**States**

| State | Shown |
|---|---|
| `greeting` | In the menu header, whenever the villager has offers |
| `cooldown` | The villager has nothing because you did their quest recently |
| `locked` | The villager has something, but you have not earned it yet |
| `no_quests` | The villager has nothing, and there is no more specific reason |

**How a line is chosen**

1. A quest's own `cooldown`/`locked` line always wins where it has one.
2. Otherwise pools for that state are read highest `priority` first. The first pool with a
   **matching conditioned line** ends the search — shadowing means shadowing, not averaging.
3. If no conditioned line matches anywhere, the unconditioned lines from every pool are the
   candidates.
4. Among the candidates, one is drawn by `weight`, **deterministically** per player, villager, day
   and state. Reopening the menu does not re-voice the villager — the same guarantee 1.4.3 gave
   offers.

Give every pool at least one unconditioned line. Without one, a villager whose personality you did
not cover falls back to the mod's flat text, which is the thing this file exists to replace.
...
| `anchor` | Any [location anchor](#location-anchors). |

At least one must be set, or the pack fails to load: a `source` that names nothing is a marker that
never appears with no way to find out why. When several are set they are tried in the order above.
...
`build_near_location`, `escort_entity`, `talk_to_profession` and every villager-targeted objective
already know where they are sending the player and need no `source`.
...
| Locale | Coverage |
|---|---|
| `en_us` | Source of truth — 2,869 keys |
| `pt_br` (Português do Brasil) | Complete — all 2,869 keys |
